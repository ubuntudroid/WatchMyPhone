#usr/bin/sh
# This scripts overrides the current Mobilis Server configuration as defined in /src/META-INF/MobilisSettings.xml and hibernate.xml by the default local configuration saved in the appropriate bak files by the a accompanied wmp_server_config.sh script. This script is normally run *after* the wmp_server_config.sh script and reverts everything back to a local setup, i.e. a local XMPP server/database. It returns the Server project back to normal development state.
